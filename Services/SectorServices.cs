...
public class SectorServices : ISectorServices
{
    ...
    {
        ICollection<Sector> sector = await _database.Sectors.ToListAsync();

        try
        {
            if (sector == null)
            throw new ArgumentNullException(nameof(sector), ExceptionMessages.ErrorArgumentNullException);
    ...
    public async Task<string> SectionsName(Sector sector)
    {
        Section section = await _database.Sections.Where(x => sector.SectionId == x.Id).FirstOrDefaultAsync();
