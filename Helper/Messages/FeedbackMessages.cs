namespace SEP_Web.Helper.Messages;
public class FeedbackMessages
{

    public const string ErrorLogin = "Erro ao realizar login.";
    public const string ErrorAccountDisable = "O usuário fornecido está com o acesso desabilitado! Contate um administrador .";
    
    public const string SuccessAdministratorRegister = "Administrador cadastrado com sucesso !";
    public const string ErrorAdministratorRegister = "Erro ao registrar o administrador.";
    public const string ErrorAdministratorList = "Erro ao obter a coleção de administradores.";
    public const string SuccessAdministratorEdit = "Administrador editado com sucesso !";
    public const string ErrorAdministratorEdit = "Erro ao editar o administrador.";
    public const string SuccessAdministratorDelete = "Administrador excluído com sucesso !";
    public const string ErrorAdministratorDelete = "Erro ao excluir o administrador.";

    public const string SuccessEvaluatorRegister = "Avaliador cadastrado com sucesso !";
    public const string ErrorEvaluatorRegister = "Erro ao registrar o avaliador.";
    public const string SuccessEvaluatorEdit = "Avaliador editado com sucesso !";
    public const string ErrorEvaluatorEdit = "Erro ao editar o avaliador.";
    public const string SuccessEvaluatorDelete = "Avaliador excluído com sucesso !";
    public const string ErrorEvaluatorDelete = "Erro ao exluir o avaliador.";
    public const string ErrorEvaluatorList = "Erro ao obter a coleção de avaliadores.";


    public const string ErrorServantList = "Erro ao obter a coleção de servidores.";
    public const string ErrorEmptyCollection = "A coleção acessada está vazia, portanto não possui registros a serem exibidos !";


    public const string ErrorInstituitionList = "Erro ao obter a coleção de órgãos.";
    public const string ErrorDivisionList = "Erro ao obter a coleção de divisões.";
    public const string ErrorSectionList = "Erro ao obter a coleção de seções.";
    public const string ErrorSectorList = "Erro ao obter a coleção de setores.";
    

    public const string ConfirmPassword = "Confirme sua senha !";
    public const string UnMatchedPassords = "As senhas não são idênticas !";
}